...
const CardsPage = () => {
  ...
  const toggleCardLock = () => {
    toast.success(selectedCard.status === 'active' ? 'Kart geçici olarak kilitlendi' : 'Kart kilidi açıldı');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Kartlarım</h1>
          <p className="text-gray-500 text-sm mt-1">{cards.length} kart</p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 border-gray-100">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-xl">Kart Detayları</CardTitle>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="bg-white">
                <DropdownMenuItem className="cursor-pointer">
                  <Settings className="h-4 w-4 mr-2" /> Kart Ayarları
                </DropdownMenuItem>
                <DropdownMenuItem className="cursor-pointer">
                  <Shield className="h-4 w-4 mr-2" /> PIN Değiştir
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className={`relative p-6 rounded-2xl text-white ${getCardGradient(selectedCard.color)} shadow-xl`}>
              ...
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-xs opacity-70">Son Kullanma</p>
            ...
            {selectedCard.limit && (
              <div className="p-4 bg-gray-50 rounded-xl">
                <div className="flex justify-between items-center mb-3">
                  <p className="text-sm text-gray-500">Kart Limiti</p>
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Kullanılan: <span className="text-gray-900 font-medium">{formatCurrency(selectedCard.used)}</span></span>
                  <span className="text-gray-500">Kalan: <span className="text-green-600 font-medium">{formatCurrency(selectedCard.available)}</span></span>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              >
                ...
                <span className="text-xs">Kart Kilitle</span>
              </Button>
              <Button
                variant="outline"
                className="flex flex-col items-center gap-2 h-auto py-4 hover:border-[#EC0000] hover:text-[#EC0000]"
              >
                ...
                <span className="text-xs">PIN Gör</span>
              </Button>
              <Button
                variant="outline"
                className="flex flex-col items-center gap-2 h-auto py-4 hover:border-[#EC0000] hover:text-[#EC0000]"
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-gray-900">Son Kart Hareketleri</h3>
